...
function GoogleButton({ onClick, loading, text }) {
    return (
        <Button
            ...
            size="lg"
            className="g-btn"
            onClick={onClick === undefined ? () => console.log("onClick not Set in Google Button") : onClick}
        >
            ...
            <span style={{ fontWeight: 600 }}>
                {!loading && <>{text===undefined?"Signin with Google":text}</>}
                {loading && <Spinner size="sm" style={{ marginLeft: "-2.5rem" }} />}
            </span>
        </Button>
    );
}
